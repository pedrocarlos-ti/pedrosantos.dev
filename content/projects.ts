...
export const projects: Project[] = [
  {
    ...
    caseStudy: {
      problem:
        "Every React/Next.js project ends up with a dozen npm scripts — `dev`, `build`, `lint`, `test`, `typecheck`, plus project-specific ones like `db:seed`. The default way to run them is either typing `npm run <name>` from memory or scrolling the scripts panel. Both break flow. I wanted something closer to a command palette: type a few letters, hit enter, done.",
      approach:
      ...
      outcome:
        "A small, focused extension that does one thing well. It's the kind of tool I built for myself first and polished because the friction it removes is one I hit dozens of times a day. The real reward is that muscle-memory moment: `Cmd+Shift+P`, `ss`, enter, and the right script is running.",
      learnings: [
